...
// spotEnabled returns the label set on workload to opt in to spot scheduling.
func spotEnabled() map[string]string {
	return map[string]string{spotEnabledLabelKey: spotEnabledLabelValue}
}

// spotAnnotations returns the spot scheduling annotation map for a workload.
func spotAnnotations(percentage, minOnDemand int) map[string]string {
	return map[string]string{
		spotConfigAnnotation: fmt.Sprintf(`{"percentage":%d,"minOnDemandReplicas":%d}`, percentage, minOnDemand),
	}
}

...
// installerScriptBaseURL returns the base URL for the pipeline-specific install-ssi.sh script,
// mirroring the pattern used in newInstallerScriptSuite (test/new-e2e/tests/installer/script).
func installerScriptBaseURL() string {
	if pipelineID, ok := os.LookupEnv("E2E_PIPELINE_ID"); ok {
		return "s3.amazonaws.com/installtesting.datad0g.com/pipeline-%s" + pipelineID
	}
	return "install.datadoghq.com"
}

// RunInstallScript overrides the base suite to ensure the ansible install method uses the
// pipeline-specific install-ssi.sh (same S3 URL pattern as newInstallerScriptSuite).
func (s *packageApmInjectSuite) RunInstallScript(params ...string) {
	if s.installMethod == InstallMethodAnsible {
		params = append(params, "DD_INSTALLER_REGISTRY_URL_INSTALLER_PACKAGE="+installerScriptBaseURL())
	}
	s.packageBaseSuite.RunInstallScript(params...)
}
